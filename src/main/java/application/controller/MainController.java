...
import application.controller.json_model.ModifyUser;
...
public class MainController {
    ...
    public Success modify(@RequestBody ModifyUser user){
        String name = user.getUser_name();
        String oldName = user.getOld_name();
        String password = user.getUser_pwd();

        //首先判断user_name是否已经存在
        boolean if_exist = userService.findUser(name);

        Student stu = userService.findStudentByName(oldName);
        Teacher tea = userService.findTeacherByName(oldName);
